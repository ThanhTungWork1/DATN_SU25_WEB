...
const OrdersStatusChart: React.FC = () => {
  ...
  console.log("OrdersStatusChart Data:", data);
      ...
      const vietnameseName = statusMap[payload[0].name] || payload[0].name;
         ...
         <div style={{
           ...
           <p style={{ 
             ...
             Tỷ lệ: {((payload[0].value / data.reduce((sum: number, item: any) => sum + item.count, 0)) * 100).toFixed(1)}%
           </p>
         </div>
       );
    }
           ...
           {payload?.map((entry: any, index: number) => (
             <div key={index} style={{ 
               ...
               <span style={{ whiteSpace: 'nowrap' }}>
                 {statusMap[entry.value] || entry.value} ({((entry.payload.count / data.reduce((sum: number, item: any) => sum + item.count, 0)) * 100).toFixed(0)}%)
               </span>
             </div>
           ))}
         </div>
