...
import { Card, Table, Button, Input, Space, Tag, Image, Row, Col, Statistic, message } from 'antd';
...
const mockInventoryData = [
  {
    ...
    price: 99000,
    variants: [
      { size: 'S', color: 'Đen', stock: 2 },
      { size: 'S', color: 'Xám', stock: 4 },
      { size: 'S', color: 'Trắng', stock: 9 },
    ],
    min_stock: 2,
    total_stock: 15,
    status: 'low_stock'
  },
  {
    ...
    price: 199000,
    variants: [
      { size: 'M', color: 'Xanh', stock: 25 },
      { size: 'L', color: 'Đỏ', stock: 18 },
    ],
    min_stock: 18,
    total_stock: 43,
    status: 'in_stock'
  ...
  {
    id: 3,
    name: 'Quần Jeans Nam Basics',
    image: 'https://via.placeholder.com/60x60',
    category: 'Quần nam',
    price: 672000,
    variants: [
      { size: '30', color: 'Xanh đậm', stock: 0 },
      { size: '32', color: 'Xanh đậm', stock: 5 },
    ],
    min_stock: 0,
    total_stock: 5,
    status: 'out_of_stock'
...
const InventoryPage: React.FC = () => {
  ...
  // Fetch real data
  const { data: statsData, isLoading: statsLoading, refetch: refetchStats } = useInventoryStats();
  const { data: inventoryData, isLoading: inventoryLoading, refetch: refetchInventory } = useInventoryList(searchText, currentPage, pageSize);
      ...
      <Card style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Space>
            ...
            <Button icon={<ReloadOutlined />} onClick={handleRefresh} loading={statsLoading || inventoryLoading}>
              Làm mới
            </Button>
          </Space>
                           <Search
             ...
             <Card>
               <Table
                 columns={columns}
                 dataSource={inventoryData?.data?.map((item: any, index: number) => ({ ...item, key: item.id })) || []}
                 pagination={{
