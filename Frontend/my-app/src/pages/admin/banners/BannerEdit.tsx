...
const BannerEdit: React.FC = () => {
  ...
  const onUpdate = async (id: number) => {
    ...
    console.log('Selected file:', files[id]);
    console.log('Has file:', !!files[id]);

    const form = new FormData();
    const file = files[id];
    if (file) {
      console.log('Adding image to FormData:', file.name, file.size);
    ...
    try {
      ...
      console.log('Sending PUT request to:', `/admin/banners/${id}`);
      // Add method spoofing for Laravel
      form.append('_method', 'PUT');
      const res = await axiosInstance.post<{ message?: string }>(`/admin/banners/${id}`, form, {
        headers: {
