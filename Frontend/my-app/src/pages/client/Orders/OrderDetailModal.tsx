...
const OrderDetailModal = ({
  ...
  const getPaymentMethodDisplay = () => {
    ...
    const checkReviewStatus = async () => {
      if (!order || !["delivered", "completed"].includes(order.status.toLowerCase())) return;
      ...
      for (const item of order.items) {
        try {
          const response = await checkReviewEligibility(item.product_id, token, order.id);
          if ((response.data as any).can_review === false && (response.data as any).reason === 'already_reviewed_for_order') {
          ...
          if (error.response?.status === 500) {
            console.warn(`Skipping review status check for product ${item.product_id} due to server error`);
            continue;
          }
        }
      }
                ...
                >
                  ...
                  <div className="text-right">
                   ...
                   {/* Nút đánh giá cho từng sản phẩm */}
                    {["delivered", "completed"].includes(order.status.toLowerCase()) && !reviewedProducts.has(item.product_id) && (
                      <button
