  ...
  if (!order) return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6">
        ...
        <button onClick={onClose} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded">Đóng</button>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="text-xl font-semibold">Chi tiết đơn hàng #{order.id}</h3>
              ...
              {order.items?.map((item: any) => (
                <div key={item.id} className="flex items-center gap-4 p-4 border rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">Đơn giá: {item.price?.toLocaleString()}₫</p>
                  </div>
