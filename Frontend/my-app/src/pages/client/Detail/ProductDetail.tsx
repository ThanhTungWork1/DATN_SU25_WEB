...
const ProductDetail = () => {
  ...
  console.log('🔍 [DEBUG] productRaw?.data:', productRaw?.data);
  
  // Thử cả 2 cách: productRaw.data hoặc productRaw trực tiếp
  const product = (productRaw as any)?.data || productRaw as Product | undefined;
  ...
  // Nếu không có image, sử dụng image mặc định
  if (!product.image) {
    console.log('🔍 [DEBUG] No image found, using default');
  ...
  // Nếu không có hover_image, sử dụng hover_image mặc định
  if (!product.hover_image) {
    console.log('🔍 [DEBUG] No hover_image found, using default');
    product.hover_image = product.image;
  }

  // Nếu không có image_url và hover_image_url, sử dụng mặc định
  if (!product.image_url) {
    console.log('🔍 [DEBUG] No image_url found, using default');
    product.image_url = product.image;
  }
  if (!product.hover_image_url) {
    console.log('🔍 [DEBUG] No hover_image_url found, using default');
    product.hover_image_url = product.hover_image;
  }
