...
export class TokenManager {
    ...
    static getToken(): string | null {
        ...
        if (isAdminContext) {
            ...
            console.log('🔑 Admin context - using admin token:', adminToken ? 'exists' : 'none');
            return adminToken || localStorage.getItem(this.USER_TOKEN_KEY) || localStorage.getItem(this.LEGACY_TOKEN_KEY);
        } else {
            // Trong client context: ưu tiên user token
            const userToken = localStorage.getItem(this.USER_TOKEN_KEY) || localStorage.getItem(this.LEGACY_TOKEN_KEY);
            console.log('🔑 Client context - using user token:', userToken ? 'exists' : 'none');
    ...
    static getUserToken(): string | null {
        return localStorage.getItem(this.USER_TOKEN_KEY) || localStorage.getItem(this.LEGACY_TOKEN_KEY);
    }
    ...
    static setToken(token: string, role: 'admin' | 'user'): void {
        ...
        if (role === 'admin') {
            ...
            console.log('✅ Admin token saved, user token preserved');
        } else {
            // Chỉ set user token, GIỮ NGUYÊN admin token  
            localStorage.setItem(this.USER_TOKEN_KEY, token);
            console.log('✅ User token saved, admin token preserved');
        }
    ...
    static clearUserToken(): void {
        ...
        console.log('🗑️ User token cleared, admin token preserved');
    ...
    static clearAdminToken(): void {
        ...
        console.log('🗑️ Admin token cleared, user token preserved');
