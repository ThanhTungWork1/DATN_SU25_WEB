...
const formatPrice = (price: string | number): string => {
  console.log("🔍 [DEBUG] formatPrice input:", price, "type:", typeof price);

  let numericPrice;
  if (typeof price === "string") {
    // Nếu là string "99.000", chuyển thành number
    // Xóa tất cả dấu chấm và dấu phẩy, sau đó parse
    const cleanPrice = price.replace(/[.,]/g, "");
    numericPrice = parseFloat(cleanPrice);
    console.log(
      "🔍 [DEBUG] Cleaned price string:",
      cleanPrice,
      "parsed:",
      numericPrice
    );
    ...
    console.log("🔍 [DEBUG] Number price:", numericPrice);
  }

  // Nếu giá quá nhỏ (dưới 1000), có thể đã bị nhân sai
  if (numericPrice < 1000 && numericPrice > 0) {
    console.log("🔍 [DEBUG] Price seems too small, multiplying by 1000");
    numericPrice = numericPrice * 1000;
  }

  const result = `${numericPrice.toLocaleString("vi-VN")}₫`;
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        ...
        console.log("🔍 [DEBUG] Product set:", res.data);
        console.log("🔍 [DEBUG] Variants set:", res.data.variants || []);
      } catch (error) {
        console.error("🔍 [DEBUG] Fetch product detail error:", error);
  ...
  if (loading) {
    ...
    return (
      <Spin
        tip="Đang tải chi tiết sản phẩm..."
  ...
  if (!product) {
    console.log("🔍 [DEBUG] No product found, rendering error message");
    return (
  ...
  console.log("🔍 [DEBUG] Rendering product detail with:", product);
  ...
  const variantColumns: TableProps<VariantWithDetails>["columns"] = [
    ...
    { title: "Kích thước", dataIndex: ["size", "name"], key: "size" },
    {
      ...
      dataIndex: "stock",
      key: "stock",
      render: (stock) => <Tag color={stock > 0 ? "blue" : "red"}>{stock}</Tag>,
    },
    {
      title: "Giá",
      dataIndex: "price",
      ...
      <Row gutter={[32, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} md={8}>
          ...
          <Image.PreviewGroup>
            ...
            >
              {product.image_url && (
                <Image width={200} src={product.image_url} alt="Ảnh chính" />
              )}
